...
function SnippetCard({ posts }: { posts: Posts }) {
  const { user } = useUser();
  const deleteSnippet = useMutation(api.posts.deletePost);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSnippet({ postId: posts._id });
    } catch (error) {
  ...
  return (
    ...
    >
      <Link href={`/community/${posts._id}`} className="h-full block">
        <div
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  >
                    <Image
                      src={`/${posts.language}.png`}
                      alt={`${posts.language} logo`}
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    {new Date(posts._creationTime).toLocaleDateString()}
                  </div>
                </div>
              </div>
              <div
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-sm text-gray-400">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="truncate max-w-[150px]">{posts.userName}</span>
                  </div>
                </div>
              </div>
